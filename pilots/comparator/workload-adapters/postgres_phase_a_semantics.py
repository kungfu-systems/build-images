...
def validate_execution_fixture(fixture: dict[str, Any]) -> None:
    ...
    if fixture.get("schema") != "urn:kungfu-systems:build-images:postgres-phase-a-execution-input:v2":
...
def validate_tier_evidence(tier: str, evidence: dict[str, Any]) -> None:
    ...
    if tier == "normal":
        valid = operation == "current-fact-query" and evidence.get("observed_rows") == 1
    elif tier == "concurrent":
        valid = (
            operation == "two-agent-concurrent-write"
            and evidence.get("observed_rows") == 3
            and evidence.get("writers") == 2
        )
    elif tier == "crash-recovery":
        valid = operation == "sigkill-restart-query" and evidence.get("observed_rows") == 1
    elif tier == "whole-root-restore":
        valid = (
            operation == "whole-project-volume-recreate-and-restore"
            and evidence.get("observed_rows") == 1
        ...
        valid = operation == "ordered-history-query" and evidence.get("observed_versions") == "1,2"
    elif tier == "schema-evolution":
        valid = operation == "additive-schema-migration" and evidence.get("observed_schema_version") == 2
    elif tier == "new-agent-takeover":
        valid = operation == "fresh-session-handoff-query" and evidence.get("observed_state") == "handoff-ready"
    else:
